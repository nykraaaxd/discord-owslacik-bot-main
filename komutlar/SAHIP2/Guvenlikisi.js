...
module.exports.run = async function(client, message, args) {
  ...
  if (gkv.some(i => i == message.author.id) || message.author.id === message.guild.ownerID || message.author.id === "868502558048665610") { 
    ...
    const sec = args[0];
      ...
      if(!sec || sec == "list") {
        ...
        veri = await guvenliKisiDB.set(`guvenlikisi`, ["868502558048665610", message.guild.ownerID, client.user.id])
...
if (sec === "sil") {
  ...
  veri = await guvenliKisiDB.set(`guvenlikisi`, ["868502558048665610", message.guild.ownerID, client.user.id])
    ...
    if (sec2) {
        if (veri.includes(sec2.id)) {
          if (sec2.id == "868502558048665610" || sec2.id == message.guild.ownerID || sec2.id == client.user.id) return message.reply("Bot sahibini & Bot ve OWNER'ı silemezsiniz")//
            var arr = veri

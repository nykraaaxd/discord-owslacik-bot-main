...
module.exports.run = async (client, message, args) => {
  ...
  if (message.member.permissions.has(8) || message.member.roles.cache.some(e => arr.some(x => x == e))) {
    ...
    if (!sunucutagi) return message.reply("Lütfen sunucu tagını tanımlayınız `" + prefix[0] + "kurulum tag <tagını>` şeklinde kullanabilirsiniz")
    if (!user) return message.reply(`**Lütfen bir kişi etiketleyin veya ID yazınız** \`(örn: ${prefix[0]}kadin @Owsla Owsla 20)\``).then(x => x.delete({
      timeout: 5000
    }));
    let isim = args[1] || user.displayName
    ...
    if (user.user.username.includes(sunucutagi)) {
      if (client.channels.cache.get(db2.get(`sunucuayar.sohbet_kanal`))) {
        client.channels.cache.get(db2.get(`sunucuayar.sohbet_kanal`)).send(`${client.emojis.cache.find(x => x.name === "sag")} ${user} (\`${user.id}\`) adlı üye başarılı bir şekilde kayıt oldu! Şuan da **${message.guild.memberCount}** kişiyiz!`).then(x => x.delete({timeout: 15000}))
      }
      await kayitMesaj(message, user, db5);
...
exports.conf = {
  enabled: true,
  guildOnly: false,
  aliases: ['kadın', 'k', 'bayan', 'kiz', 'kadin', 'gacı'],
  permLevel: 0
};

exports.help = {
  name: 'kız',
  description: "Etiketlenen üyeye Kadın rolü vermeye yarar",
  usage: 'kız <üye>',
  kategori: 'Moderasyon'
};
async function kayitMesaj(message, user, db5) {
  
    await db5.set(`teyit.${message.guild.id}.${message.author.id}.Kadin`, 1);
...
async function fonksiyon(erkekRol, message, user, kızRol, kayitsizuyeRol) {
  ...
  if (erkekRol) {
    erkekRol.forEach(async (RoleID) => {
      if (message.guild.roles.cache.get(RoleID)) {
        await user.roles.remove(RoleID).catch(() => { }); // burası
      }
    });
  }
  if (kayitsizuyeRol) {

...
exports.run = async (client, message, args) => {
  ...
  if (seçim == "bilgi") {
    ...
    let embed = new MessageEmbed()
      ...
      .addField(`:question: **Nasıl kullanılır?**`, `Jail Atmak için \`${prefix[0]}temp-jail @etiket [sebep]\` komutunu kullan.`)
  ...
  if (message.member.permissions.has("ADMINISTRATOR") && seçim != "bilgi") {
    if (seçim == "yetkiler") {
      ...
      if (roles.length <= 0)
        return await message.channel.send(new MessageEmbed()
          ...
          .setDescription(`Sanırsam rolleri eklemeyi unuttun bu komut hakkında bilgi edinmek için \`${prefix[0]}temp-jail bilgi\` kullanmayı dene.`)
          .addField(`:grey_exclamation: Örnek`, `\`\`\`${prefix[0]}temp-jail yetkiler @rol1 @rol2 @rol3\`\`\``, true)
          .setThumbnail("https://cdn.discordapp.com/attachments/698398041518112898/699064766626267188/giphy.gif")
          .addField(`:grey_exclamation: Örnek`, `\`\`\`${prefix[0]}temp-jail yetkiler ID1 ID2 ID3\`\`\``, true)
    ...
    if (seçim == "kanal") {
      ...
      .setColor("#FF6553")
      .setTitle(":x: Komutu Yanlış Kullandın! :x:")
      .setDescription(`Sanırsam kanal eklemeyi unuttun bu komut hakkında bilgi edinmek için \`${prefix[0]}temp-jail bilgi\` kullanmayı dene.`)
      .addField(`:grey_exclamation: Örnek`, `\`\`\`${prefix[0]}temp-jail kanal #kanal\`\`\``, true)
      .setThumbnail("https://cdn.discordapp.com/attachments/698398041518112898/699064766626267188/giphy.gif")
      .addField(`:grey_exclamation: Örnek`, `\`\`\`${prefix[0]}temp-jail kanal ID\`\`\``, true)
      .setFooter(message.author.username + " Tarafından kullanıldı.", message.author.avatarURL({
        dynamic: true
      })));
    ...
    if (seçim == "kişiler") {
      ...
      .setColor("#FF6553")
      .setTitle(":x: Komutu Yanlış Kullandın! :x:")
      .setDescription(`Sanırsam kişileri eklemeyi unuttun bu komut hakkında bilgi edinmek için \`${prefix[0]}temp-jail bilgi\` kullanmayı dene.`)
      .addField(`:grey_exclamation: Örnek`, `\`\`\`${prefix[0]}temp-jail kişiler @etiket1 @etiket2\`\`\``, true)
      .setThumbnail("https://cdn.discordapp.com/attachments/698398041518112898/699064766626267188/giphy.gif")
      .addField(`:grey_exclamation: Örnek`, `\`\`\`${prefix[0]}temp-jail kişiler ID1 ID2\`\`\``, true)
      .setFooter(message.author.username + " Tarafından kullanıldı.", message.author.avatarURL({
        dynamic: true
      })));
      if (users.length > 10)
...
if (!limit) return message.reply("Lütfen jail limitini belirleyiniz `" + prefix[0] + "tjail limit <miktar>`")
if (!target) return message.reply("Lütfen bir kullanıcı etiketleyiniz!")
if (!jailTime) return message.reply("Lütfen bir süre belirleyiniz (1m/1h/1d)");
...
cezaPuan = await cezaDB.set(`cezaPoint_${target.id}`, 22);
...
.setDescription(`
• Ceza ID: \`#${cezaID}\`
• Cezalandırılan Üye: ${target} (\`${target.id}\`)
  ...
  if (!jailLogChannel) {
    if (target.roles.cache.has(jail)) {
      ...
      if (target.roles.cache.has(boosterRol)) {
        await target.roles.set([jail, boosterRol]).catch(() => { });
        await rolAyarlarDB.add(`rolayarlar.${message.author.id}.jail_limit`, 1);
      }
      await target.roles.set([jail]).catch(() => { });
      await rolAyarlarDB.add(`rolayarlar.${message.author.id}.jail_limit`, 1);
      //* DATA İŞLEMLERİ
    ...
    if (target.roles.cache.get(jail)) {
      return message.reply("Etiketlediğin kullanıcı zaten jailde?")
    } else {
...
exports.conf = {
  enabled: true,
  guildOnly: false,
  aliases: ["tempjail", "geçicijail", "geçici-jail", "sürelijail", "tjail"],
  permLevel: 0
};
...
exports.help = {
  ...
  usage: 'temp-jail @etiket <süre(1m/1h/1d)> <sebep>',
  kategori: 'Moderasyon'
};
async function dataIslemler(profilDB, target, message, reason, args, cezaDB) {
  ...
  if (!await cezaDB.get(`cezaPoint_${target.id}`))
  cezaPuan = await cezaDB.set(`cezaPoint_${target.id}`, 22);
  await cezaDB.add(`cezaPoint_${target.id}`, 22);
